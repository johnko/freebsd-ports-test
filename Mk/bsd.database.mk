...
_MYSQL-GALERA!=	${LOCALBASE}/bin/mysql --version | ${GREP} MySQL-Galera | wc -l
_MARIADB-GALERA!=	${LOCALBASE}/bin/mysql --version | ${GREP} MariaDB-Galera | wc -l

.if ${_PERCONA} == 1
_MYSQL_VER=	${_MYSQL}p
.elif ${_MARIADB} == 1
_MYSQL_VER=	${_MYSQL}m
.elif ${_MYSQL-GALERA} == 1
_MYSQL_VER=	${_MYSQL}g
.elif ${_MARIADB-GALERA} == 1
_MYSQL_VER=	${_MYSQL}mg
.else
_MYSQL_VER=	${_MYSQL}
.endif
.endif
...
.elif (${MYSQL_VER} == "55g")
_MYSQL_CLIENT=	databases/mysql55-galera-client
_MYSQL_SERVER=	databases/mysql55-galera-server
.elif (${MYSQL_VER} == "56g")
_MYSQL_CLIENT=	databases/mysql56-galera-client
_MYSQL_SERVER=	databases/mysql56-galera-server
.elif (${MYSQL_VER} == "55mg")
_MYSQL_CLIENT=	databases/mariadb55-galera-client
_MYSQL_SERVER=	databases/mariadb55-galera-server
.elif (${MYSQL_VER} == "56p")
